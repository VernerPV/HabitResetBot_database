...
def start(message):
    ...
    bot.reply_to(message, f"Привет, {username}!Я бот-Макс. Я буду помогать тебе менять привычки . Готов? Жми меню.",
                 reply_markup=markup)  # Приветствуем пользователя

    db_object.execute(
        f"SELECT user_id FROM users WHERE user_id = {user_id}")  # Проверяем есть ли пользователь в БД и если нет то добавляем
...
@bot.message_handler(func=lambda message: True,
                     content_types=["text"])  # Отслеживаем все сообщения пользователя и  увеличиваем счетчик
